...
from chris_plugin.parameters import (
    serialize_store_action,
    get_param_type,
    serialize,
    should_include,
)
...
def test_required_int(parser: ArgumentParser):
    action = parser.add_argument(
        "-n", "--number", type=int, required=True, default=1, help="an integer value"
    )
...
def test_optional_int(parser: ArgumentParser):
    action = parser.add_argument("-a", "--aa", type=int, required=False, help="aaa")
    expected = ParameterSpec(
        name="aa",
        type="int",
        optional=True,
...
def test_serialize_choices(parser: ArgumentParser):
    p = parser.add_argument(
        "-a", "--apple", choices=("red", "green"), help="Crispy fruit"
    )
    expected = ParameterSpec(
        name="apple",
        type="str",
...
def test_serialize_choices_num(parser: ArgumentParser):
    p = parser.add_argument(
        "-n", "--num", choices=range(1, 9, 3), default=4, help="Pick a lucky one"
    )
    expected = ParameterSpec(
        name="num",
        type="int",
